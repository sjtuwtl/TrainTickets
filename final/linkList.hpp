...
public:
	...
	virtual void write(fstream &fout) {
...
class listString: public list<string> {
public:
	...
	void write(fstream &fout) {
		fout.write(reinterpret_cast<const char *> (&curSize), sizeof curSize);
		for (node *p = tail -> prev; p != head; p = p -> prev) {
			littleTool::writeString(fout, p -> data);	
		}
	}
};

class listTicket: public list<ticket> {
public:
	void read(fstream &fin) {
		clear();
		size_t tmpLen;
